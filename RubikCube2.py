...
def drawPatches(points, nvertices, drawAllPatches, cubeID) :
  ...
  for face in faces :
    ...
    # Set colour of patches according to orientation and cubeID
    if cubeID == 1 :
      if orientation == 0 :
    ...
    if not((orientation == 1 and drawAllPatches == 0) or orientation == -2) :
      ri.Patch("bilinear",{'P':newFaceVerts})


def drawPiece(drawAllPatches, cubeID) :
  ...
  ri.SubdivisionMesh("catmull-clark", 
                      ...
                      {ri.P:points})
  drawPatches(points, nvertices, drawAllPatches, cubeID)
...
for x in range (-2, 1) :
  for z in range(0, 3) :
    for y in range (0, 2) :
      ...
      ri.Translate(x,y,z)
      drawAllPatches = 0
      drawPiece(drawAllPatches, cubeID)
      ri.TransformEnd()
# Top rotated pieces
ri.TransformBegin()
ri.Rotate(15, 0, 1, 0)
ri.Translate(-0.1, 0, -0.4)
for x in range (-2, 1) :
  for z in range(0, 3) :
    ri.TransformBegin()
    ri.Translate(x,2,z)
...
ppoints = [-30, -0.5, -30, -30, -0.5, 30, 30, -0.5, -30, 30, -0.5, 30]
ri.TransformBegin()
